...
const AdminOrgApprovals = () => {
  ...
  const fetchPendingOrganizations = useCallback(async () => {
    ...
    try {
      ...
      setActionError(t('adminApprovals.errLoad') + error);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleApprove = async (id: number) => {
    ...
    try {
      ...
      setActionError(getErrorMessage(error) || t('adminApprovals.errApprove'));
    }
  };
    ...
    <div className="space-y-8 font-['Inter',sans-serif]">
      <div>
        <h2 className="text-3xl font-bold tracking-tight text-[#002045]">
          {t('adminApprovals.title')}
        </h2>
        <p className="mt-1 text-sm font-medium text-[#74777f]">{t('adminApprovals.subtitle')}</p>
      </div>
      ...
      <div className="flex flex-col overflow-hidden rounded-2xl border border-[#dee3e8] bg-white shadow-[0px_4px_6px_rgba(26,54,93,0.04)]">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center bg-[#f5faff] p-16 text-center">
            ...
            <p className="mb-1 text-lg font-bold text-[#002045]">
              {t('adminApprovals.emptyQueue')}
            </p>
            <p className="text-sm font-medium text-[#74777f]">{t('adminApprovals.noPending')}</p>
          </div>
        ) : (
          <>
                ...
                <thead>
                  <tr className="border-b border-[#dee3e8] bg-[#eff4f9]">
                    <th className="px-6 py-4 text-xs font-bold tracking-wider text-[#43474e] uppercase">
                      {t('adminApprovals.colDetails')}
                    </th>
                    <th className="px-6 py-4 text-xs font-bold tracking-wider text-[#43474e] uppercase">
                      {t('adminApprovals.colTaxId')}
                    </th>
                    <th className="px-6 py-4 text-xs font-bold tracking-wider text-[#43474e] uppercase">
                      {t('adminApprovals.colContact')}
                    </th>
                    <th className="px-6 py-4 text-right text-xs font-bold tracking-wider text-[#43474e] uppercase">
                      {t('adminApprovals.colAction')}
                    </th>
                  </tr>
                </thead>
